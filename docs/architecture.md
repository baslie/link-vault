...
1. Настройка проекта Next.js + Supabase + аутентификация.
2. Реализация таблицы ссылок с добавлением/редактированием/удалением.
3. Система тегов и фильтрации (AND логика).
4. Поиск по всем полям.
5. Пагинация (20/50/100/500).
6. Темная/светлая тема.
7. Мобильная адаптация.

### Этап 2: Управление данными
1. Импорт/экспорт CSV с предпросмотром и обработкой ошибок.
2. Автоматический сбор метаданных и фавиконок.
3. Настройки профиля, управление тегами (редактирование, удаление, выбор цвета).

### Этап 3: Администрирование и аналитика
1. Простая админ-панель со статистикой.
2. Подключение аналитики (Яндекс.Метрика, GA).
3. Мониторинг производительности и логирование.

### Этап 4: Дополнительные улучшения
1. Массовые операции (bulk delete) — если хватит времени.
2. Проверка доступности ссылок (фоновые задачи).
3. Дополнительные интеграции (браузерное расширение, API) — отложено по брифу.
